from cloudshell.cp.core.request_actions.models import VmDetailsProperty, VmDetailsData
...
class VmDetailsProvider(object):
    ...
    def _get_internal_external_services_set(self, services):
        if services:
            internal_service = \
                first_or_default(lambda x: x.metadata.labels.get(TagsService.INTERNAL_SERVICE, False), services)

            external_service = \
                first_or_default(lambda x: x.metadata.labels.get(TagsService.EXTERNAL_SERVICE, False), services)
